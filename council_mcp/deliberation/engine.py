...
class DeliberationEngine:
    ...
    def _participant_identifier(self, participant: Participant) -> str:
        """Return the user-visible identifier for a participant."""
    ...
    def _build_participant_prompt(
        ...
        """Build the prompt for one participant, applying persona text if present."""
        if participant.persona_prompt:
            persona_prompt = participant.persona_prompt.strip()
            if persona_prompt:
...
"""
        ...
        async def invoke_participant(participant: Participant) -> tuple[Participant, str]:
            ...
            participant_label = self._participant_identifier(participant)
            participant_prompt = self._build_participant_prompt(
                participant, enhanced_prompt
            )

            reasoning_info = f", reasoning_effort={participant.reasoning_effort}" if participant.reasoning_effort else ""
            logger.info(
                f"Round {round_num}: Invoking {participant_label} "
            ...
            try:
                ...
                logger.info(
                    f"Round {round_num}: Received response from {participant_label}, "
                ...
                if retry_config.enabled and self._needs_vote_retry(response_text):
                    for retry_attempt in range(retry_config.max_retries):
                        logger.info(
                            f"Round {round_num}: Retrying vote for {participant_label} "
                        ...
                        try:
                            ...
                            logger.info(
                                f"Round {round_num}: Retry response from {participant_label}, "
                            ...
                            if not self._needs_vote_retry(retry_response):
                                logger.info(
                                    f"Round {round_num}: Vote retry successful for {participant_label}"
                                )
                                break
                            else:
                                logger.warning(
                            ...
                            logger.warning(
                                f"Round {round_num}: Vote retry failed for {participant_label}: {retry_error}"
                            )
                ...
                if tool_requests:
                    ...
                    for tool_request in tool_requests:
                        ...
                        execution_record = ToolExecutionRecord(
                            ...
                            requested_by=self._participant_identifier(participant),
...
- Tool results are visible to all participants in subsequent rounds"""
    ...
    async def execute(self, request: "DeliberateRequest") -> "DeliberationResult":
        ...
        model_list = [self._participant_identifier(p) for p in request.participants]
        ...
        for round_num in range(1, rounds_to_execute + 1):
            ...
            try:
                ...
                round_responses = [
                    RoundResponse(
                        round=round_num,
                        participant=self._participant_identifier(p),
                ...
                progress_logger.warning(
                    f"   ROUND {round_num} CANCELLED - request interrupted"
                )
            ...
            progress_logger.info(
                ...
                f"Success: {len(successful)}/{len(round_responses)}"
            )
            for r in round_responses:
                if r.response.startswith("[ERROR"):
                    progress_logger.error(
                        f"   ERROR {r.participant}: {r.response[:100]}"
                    )
                    issues_encountered.append(f"Round {round_num}: {r.participant} - {r.response[:50]}")
                else:
                    progress_logger.info(
                        f"   OK {r.participant}: {len(r.response)} chars"
                    )
        ...
        participant_ids = [self._participant_identifier(p) for p in request.participants]
        ...
        progress_logger.info("-" * 70)
        progress_logger.info(
            f"DELIBERATION COMPLETE | Time: {total_elapsed:.1f}s | Rounds: "
            f"{actual_rounds_completed}/{rounds_to_execute}"
        )
