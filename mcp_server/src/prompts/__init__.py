...
def build_processing_process_prompt(
    pir: str,
    collected_data: str,
...
def build_processing_modify_prompt(
    ...
    return f"""You are a professional threat intelligence analyst. Apply the requested modifications to an existing processing result.
...
## Instructions
1. Apply only the requested changes — do not re-process data or add new analysis
2. Preserve all content that was not mentioned in the modification request
3. Keep the same JSON structure as the existing result

## Output Format
Return ONLY valid JSON. No preamble, no explanation, no markdown fences. Use the same structure as the existing result."""
