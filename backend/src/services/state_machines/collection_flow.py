...
async def _read_collected(session_id: str, uow=None) -> dict | None:
    ...
    if uow is None:
        raise RuntimeError(
            f"[CollectionFlow] UoW required to read collected data for {session_id}"
        )
    try:
        ...
        logger.exception(
            f"[CollectionFlow] Failed to read collected data from DB for {session_id}"
        )
        return None
...
async def _write_collected(session_id: str, pir: str, raw_data: str, uow=None) -> None:
    ...
    if uow is None:
        raise RuntimeError(
            f"[CollectionFlow] UoW required to persist collected data for {session_id}"
        )
...
class CollectionFlow(BasePhaseFlow):
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "direction_context": self.direction_context.model_dump()
    ...
    def from_dict(cls, data: dict, research_logger=None) -> "CollectionFlow":
        ...
        flow = cls(
            ...
            pir=data["pir"],
            direction_context=DialogueContext.model_validate(data["direction_context"])
    ...
    async def initialize(self, collection_service, uow=None) -> DialogueResponse:  # noqa: ARG002
        ...
        try:
            ...
            logger.error(
                f"[Session {self.session_id}] Failed to generate collection plan: {e}"
            )
            return DialogueResponse(
                action=DialogueAction.ERROR,
                content="Failed to generate collection plan",
            )

        self.state = CollectionState.PLAN_CONFIRMING
        return DialogueResponse(
            action=DialogueAction.SHOW_PLAN, content=self.collection_plan or ""
        )
    ...
    async def process_user_message(  # type: ignore[override]
        ...
        # COMPLETE
            ...
            return DialogueResponse(
                action=DialogueAction.COMPLETE, content="Collection phase completed"
            )

    async def handle_plan_confirming(
        self,
        user_message,
        collection_service,
        approved,
    ...
    async def handle_collecting(
        ...
        try:
            ...
            self.state = CollectionState.PLAN_CONFIRMING
            return DialogueResponse(
                action=DialogueAction.ERROR, content="Collection failed"
            )
        ...
        if approved:
            ...
            if self.pending_reasoning_log and self.research_logger:
                ...
                self.pending_reasoning_log.final_approved_content = (
                    json.dumps(collected) if collected else ""
                )
                self.pending_reasoning_log.timestamps["collection_approved"] = (
                    datetime.now().isoformat()
                )
                self.research_logger.write_reasoning_log(self.pending_reasoning_log)
            self.state = CollectionState.COMPLETE
            return DialogueResponse(
                action=DialogueAction.COMPLETE, content="Collection phase complete"
            )
            ...
            self.state = CollectionState.COLLECTING
            return DialogueResponse(
                action=DialogueAction.START_COLLECTING,
                content=json.dumps(self.selected_sources),
            )
            ...
            raw = (
                ...
                if collected and collected.get("attempts")
                else ""
            )
            try:
                ...
                return DialogueResponse(
                    action=DialogueAction.ERROR,
                    content="Failed to modify collection summary",
                )
