...
class PMESIIEntity(BaseModel):
    ...
    sources: list[DataSource] = Field(
        ..., description="All sources that contributed to this entity"
    )
    confidence: int = Field(
        ..., ge=0, le=100, description="How well this finding is supported (0-100)"
    )
    relevant_to: list[str] = Field(
        default_factory=list, description="PIR IDs this entity helps answer"
    )
    tags: list[str] = Field(
        default_factory=list, description="Free-form tags for relations and context"
    )
