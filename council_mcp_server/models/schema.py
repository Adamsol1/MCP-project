...
class Participant(BaseModel):
    ...
    display_name: Optional[str] = Field(
        default=None,
        description=(
            "Optional user-facing participant name. When provided, this replaces "
            "the raw model identifier in transcripts, round outputs, and result summaries."
        ),
    )
    persona_prompt: Optional[str] = Field(
        default=None,
        description=(
            "Optional participant-specific persona instruction prepended before the "
            "shared deliberation prompt for this participant only."
        ),
    )
